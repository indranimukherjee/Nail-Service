        ...
        public ActionResult Index(int SBId = 0)
        {
            ...
            if (SBId > 0)
            {
                ...
                if (serviceBillDeskItem != null)
                {
                    ...
                    item.ServiceName = serviceBillDeskItem.ServiceMaster.ServiceType;
                    item.serviceVeriation = serviceBillDeskItem.ServiceVeriation.ServiceVariation;
                    item.timeDuration = serviceBillDeskItem.ServiceDuration;
                    item.price = serviceBillDeskItem.ServicePrice;
                    item.NoOfPerson = serviceBillDeskItem.ServiceQuantity;
                    item.ServiceBillDeskId = serviceBillDeskItem.ID;
